...
@Component({
  selector: 'az-shop-product-stock-status-under',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './shop-product-stock-status-under.component.html',
  styleUrl: './shop-product-stock-status-under.component.scss'
})
export class ShopProductStockStatusUnderComponent {
shopId: number;
  ...
  loadShopProductStockStatus = async () => {
    ...
    await this.http.get<IShopProduct[]>(API_URL + '/shop_products/get_under_stock_by_shop?shop_id=' + this.shopId, options)
      .toPromise()
      .then(
        data => {
