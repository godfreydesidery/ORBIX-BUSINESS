...
@Component({
  selector: 'az-fast-moving-dineables-report',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './fast-moving-dineables-report.component.html',
  styleUrl: './fast-moving-dineables-report.component.scss'
})
export class FastMovingDineablesReportComponent {
documentHeader! : any
  ...
  constructor(
    ...
    ngOnInit() {
      ...
      // this.getFastMovingDineablesByDate(this.from, this.to);
      // this.getBranchUserNames();
    }

    fastMovingDineables : IFastMovingDineables[] = []
    total : number = 0

  async getFastMovingDineablesByDate(from : Date | string | null, to : Date | string | null) {
    ...
    this.fastMovingDineables = []
    this.total = 0
    

    await this.http.post<IFastMovingDineables[]>(API_URL+'/sales_reports/get_fast_moving_dineables_report_by_dates', args, options)
        .toPromise()
        .then(
          data => {

            this.fastMovingDineables = data!

            var sn = 1
            this.total = 0
            this.fastMovingDineables.forEach(element => {
              element.sn = sn
  ...
  printReport = async () => {
    ...
    var header = ''
    var footer = ''
    var title  = 'Fast Moving Dineables Report'
    ...
    report.push([
      ...
      { text: 'Dineable Name', fontSize: 8, alignment: 'left', fillColor: '#ffffff', bold: true },
...
export interface IFastMovingDineables {
  sn : any
  dineableName : string
  qty : string
  amount : string
