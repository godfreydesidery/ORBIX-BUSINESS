...
export class RestaurantSalesOrderComponent {
  ...
  onAgentChange(event: Event) {
    ...
    this.selectedAgentId = target.value; // Already updated by ngModel, but you can still read it
  }
  ...
  async getAvailableAgentsByRestaurant() {
    let options = {
      headers: new HttpHeaders().set('Authorization', 'Bearer ' + this.auth.user.access_token)
    }
    this.restaurantAgents = []

    await this.http.get<IRestaurantAgent[]>(API_URL + '/restaurants/get_available_agents?restaurant_id=' + this.restaurantId, options)
      .toPromise()
      .then(
        data => {
          var sn = 1
  ...
  verifyDetail(id: any){
    ...
    this.msg.showErrorMessage3("Not Implemented")
  }
