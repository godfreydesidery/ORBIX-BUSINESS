...
@Component({
  selector: 'az-shop-product-stock-status-out',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './shop-product-stock-status-out.component.html',
...
export class ShopProductStockStatusOutComponent {
  ...
  loadShopProductStockStatus = async () => {
    ...
    await this.http.get<IShopProduct[]>(API_URL + '/shop_products/get_out_of_stock_by_shop?shop_id=' + this.shopId, options)
      .toPromise()
      .then(
        data => {
