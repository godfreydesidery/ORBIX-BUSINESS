...
@Component({
  ...
  styleUrl: './grn-report.component.scss'
})
...
documentHeader! : any
  ...
  async getStockLogByDate(from : Date | string | null, to : Date | string | null) {
    ...
    await this.http.post<ILpoReport[]>(API_URL+'/grns/get_grn_report_by_dates', args, options)
        .toPromise()
        .then(
          data => {
  ...
  printReport = async () => {
    ...
    this.fastMovingProducts.forEach(element => {
      ...
      report.push([
        ...
        { text: element.receivedQty || '', fontSize: 9, alignment: 'left', fillColor: '#ffffff', bold: false },
