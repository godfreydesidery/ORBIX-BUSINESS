...
export class MyJobsComponent {
  ...
  documentHeader! : any
      ...
      ngOnInit(){
        this.getAllCheckedInMaintenancesWithOpenJobsAndMine()   
      ...
      async getAllCheckedInMaintenancesWithOpenJobsAndMine(){
        let options = {
          headers: new HttpHeaders().set('Authorization', 'Bearer '+this.auth.user.access_token)
        }
        this.maintenances = []
    
        await this.http.get<IMaintenance[]>(API_URL+'/maintenances/get_all_checked_in_with_open_jobs_and_mine', options)
        .toPromise()
        .then(
          data => {
            var sn = 1
    ...
    async createOrLoadMaintenanceJobCard(maintenanceId: any) {
      ...
      await this.http.post<IMaintenanceJobCard>(API_URL + '/maintenances/create_maintenance_job_card_and_mine', maintenance, options)
        .toPromise()
        .then(
          data => {
