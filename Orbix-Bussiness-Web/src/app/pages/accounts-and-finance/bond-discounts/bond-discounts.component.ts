...
@Component({
  ...
  templateUrl: './bond-discounts.component.html',
...
page: number = 1; // Initialize the current page to 1
  ...
  async discounts(serviceId: any, serviceName: string) {
    ...
    await this.router.navigate(['app/accounts-and-finance/discounts'], {
      queryParams: { service_id: serviceId, service_name: serviceName }
    });
  ...
  async getBondItemBillReceivables(bondItemId: any) {
    ...
    await this.http.get<IBondItemBillReceivable[]>(API_URL + '/bond_items/get_bond_item_bill_receivables?bond_item_id=' + bondItemId, options)
      .toPromise()
      .then(
        data => {
          var sn = 1
  ...
  async bondItemBilling(bondItemId: any) {
    ...
    localStorage.setItem('bond-item-id', '');
    localStorage.setItem('bond-item-id', bondItemId);
  ...
  showStorageData(data: IBondItem) {
  ...
  async getStorageGoodReleaseDetail(bondItemId: any) {
    let options = {
      headers: new HttpHeaders().set('Authorization', 'Bearer ' + this.auth.user.access_token)
    }
    ...
    await this.http.get<IBondItemGoodReleaseDetail>(API_URL + '/storage_good_releases/get_storage_good_release_detail?bond_item_id=' + bondItemId, options)
      .toPromise()
      .then(
        data => {

          this.currentStorageId = bondItemId
  ...
  async createStorageGoodRelease() {
    ...
    var storageGoodRelease = {
      bondItemId: this.currentStorageId,
      qty: this.qtyToRelease,

    }

    await this.http.post<IBondItemGoodReleaseDetail>(API_URL + '/storage_good_releases/create_storage_good_release', storageGoodRelease, options)
      .toPromise()
      .then(
        data => {
          //this.getStorageBillReceivables(this.bondItemId)
  ...
  async checkOut(id: any): Promise<void> {
    ...
    await this.http.post<IBondItem>(API_URL + '/bonds/check_out', storage, options)
      .toPromise()
      .then(
        data => {
  ...
  async getLastBillingDate(bondItemId: any) {
    ...
    await this.http.get<IModel>(API_URL + '/bonds/get_last_storage_bill_date?id=' + bondItemId, options)
      .toPromise()
      .then(
        data => {
          console.log(data)
