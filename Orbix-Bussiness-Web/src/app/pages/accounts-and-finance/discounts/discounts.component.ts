...
// Parking attributes
  ...
  ngOnInit() {
    ...
    this.loadDiscounts(this.serviceId, this.serviceName)
    // this.getParkingBillReceivables(this.parkingId)
    // this.getParkingServiceBillReceivables(this.parkingId)
  }

  async loadDiscounts(serviceId : any, serviceName : string){
    let options = {
    ...
    await this.http.get<IDiscount[]>(API_URL + '/discount_requests?service_id=' + serviceId + '&service_name=' + serviceName, options)
    .toPromise()
    .then(
      data => {
        this.discounts = data!
  ...
  async getDiscount(id : any) {
    
      let options = {
        headers: new HttpHeaders().set('Authorization', 'Bearer ' + this.auth.user.access_token)
      }

      await this.http.get<IDiscount>(API_URL + '/discount_requests/get?id=' + id, options)
        .toPromise()
        .then(
          data => {
            this.discountId = id
  ...
  async approve(){
    let options = {
      ...
      var discountRequest = {
        id : this.discountId,
        comments: this.discountComments
      }

      await this.http.post<IDiscount>(API_URL + '/discount_requests/approve', discountRequest, options)
      .toPromise()
      .then(
        data => {


          this.msg.showSuccessMessage("Approved")
          this.loadDiscounts(this.serviceId, this.serviceName)
  ...
  async reject(){
    let options = {
      ...
      await this.http.post<IDiscount>(API_URL + '/discount_requests/reject', discountRequest, options)
      .toPromise()
      .then(
        data => {

          this.msg.showSuccessMessage("Rejected")
          this.loadDiscounts(this.serviceId, this.serviceName)
        }
      )
      .catch(
...
function List<T>(arg0: string, options: { headers: HttpHeaders; }) {
